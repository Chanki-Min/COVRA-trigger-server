...
module.exports = function() {
    ...
    this.GisaidCrawler = class {
        ...
        constructor(fromSubmission, toSubmission, gisaidCovMetaData) {
            for(let i=0; i<arguments.length; i++) {
                if(arguments[i] instanceof Date) {
                    arguments[i] = arguments[i].toISOString().slice(0, 10);
                }
            }
            this.fromSubmission = arguments[0];
            this.toSubmission = arguments[1];
            this.gisaidCovMetaData = gisaidCovMetaData;
        }

        async crawlGisaid() {
            await this.startPuppeteer();
            await this.goToUrl(process.env.GISAID_URL);
            await this.performLogin(process.env.GISAID_ID, process.env.GISAID_PW);

            await this.goToBrowsePage();
            await this.page.waitFor(5000);
            await this.insertSubmissionDate(this.fromSubmission, this.toSubmission);

            await this.goToLastPage();
            if(! await this.checkIsDataExist()) {
                console.log("No data exists");
                return;
            }

            while(true) {
                const dataList = await this.crawlCurrentPage();
                this.insertDataToDB(dataList);

                const nextPageIdx = await this.goToNextPageAndReturnIndex(process.env.NEXT_PAGE_DELAY_SECONDS);
        ...
        async performLogin(base64Id, base64Pw) {
            //로그인 절차 시작
            await this.page.type('#elogin', base64ToStr(base64Id));
            await this.page.type('#epassword',base64ToStr(base64Pw));
            await this.page.click('#login > div:nth-child(2) > input.form_button_submit')
            //로그인 완료 대기
            await this.page.waitForNavigation();
        }

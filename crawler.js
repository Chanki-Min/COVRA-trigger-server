...
module.exports = function() {
    this.GisaidCrawler = class {
        ...
        async crawlGisaid() {
            ...
            const csvFileBuffer = await this.downloadGisaidCsv(process.env.GISAID_DATA_PATH);
        ...
        async downloadGisaidCsv(dirPath) {
            const downloadPageButton = await this.page.$x('//div[@class="sys-actionbar-action" and text()="Downloads"]');
            ...
            const downloadFileButton = (await downloadFrame.$x('//div[@class="downicon" and ./div[text()="nextmeta"]]'))[0];
            const fileName = (await downloadFileButton.getProperty('title'))._remoteObject.value;
            console.log(`current fileName = ${fileName}`)


            await this.page._client.send('Page.setDownloadBehavior', { behavior: 'allow', downloadPath: dirPath } );
